...
class _HtmlFlexRenderObject extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, HtmlFlexParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, HtmlFlexParentData>
    implements flutter.RenderFlex {
  _HtmlFlexRenderObject({
  ...
  Offset _getOffset(double main, double cross) =>
      direction == Axis.horizontal ? Offset(main, cross) : Offset(cross, main);
  ...
  void performLayout() {
    ...
    while (child != null) {
      ...
      totalMainSize += _getMainSize(child.size);
      maxCrossSize = math.max(maxCrossSize, _getCrossSize(child.size));
      childCount++;
      final HtmlFlexParentData childParentData =
          child.parentData! as HtmlFlexParentData;
      child = childParentData.nextSibling;
    }

    if (childCount > 1) {
      totalMainSize += spacing * (childCount - 1);
    }
    ...
    while (child != null) {
      ...
      final bool isLastChild = childParentData.nextSibling == null;
      final double spacingToApply = isLastChild ? 0.0 : spacing;
      ...
      if (flipMainAxis) {
        childMainPosition -= spacingToApply;
      } else {
        childMainPosition +=
            _getMainSize(child.size) + spacingToApply;
      }
